...
const RelatedComicsSlider = ({ title }) => {
  ...
  useEffect(() => {
    const fetchRelatedComics = async () => {
      ...
      setError(null);
      try {
        const comicsData = await fetchComicsByTitle(title);
  ...
  return (
    ...
    <div className="slider-container">
        <h2 className='slider-title'>Related Comics You’ll Love</h2>
      <div className="slider">
        {comics.map((comic, index) => (
          ...
          >
            ...
            <button className='see-detail' onClick={() => handleProductDetailClick(comic.id)}>View Details</button>
          </div>
        ))}
      </div>
...
export default RelatedComicsSlider;
